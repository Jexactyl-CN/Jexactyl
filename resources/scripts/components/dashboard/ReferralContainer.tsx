        ...
        <PageContentBlock title={'推广'}>
            ...
            <Container className={'j-up lg:grid lg:grid-cols-3 my-10'}>
                ...
                <ContentBox title={'已邀请的用户'} css={tw`mt-8 sm:mt-0 sm:ml-8`}>
                    <SpinnerOverlay visible={loading} />
                    {activity.length === 0 ? (
                        <p css={tw`text-center my-2`}>{!loading && '此帐户不存在任何推广活动。'}</p>
                    ) : (
                        activity.map((act, index) => (
                            ...
                            >
                                ...
                                <div css={tw`ml-4 flex-1 overflow-hidden`}>
                                    ...
                                    <p css={tw`text-2xs text-neutral-300 uppercase`}>
                                        使用于：&nbsp;
                                        {act.createdAt ? format(act.createdAt, 'yyyy , MMM do HH:mm') : 'Never'}
                                    </p>
                                    <p css={tw`text-2xs text-neutral-300 uppercase`}>所用的代码：&nbsp;{act.code}</p>
                                </div>
                            </GreyRowBox>
                        ))
                    )}
